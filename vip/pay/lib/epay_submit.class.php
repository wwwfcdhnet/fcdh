...
class AlipaySubmit {
     ...
     * @return 提交表单HTML文本
     */
	function buildRequestForm($para_temp, $method='POST', $button_name='手动跳转') {
		//待请求参数数组
		$para = $this->buildRequestPara($para_temp);
	//	print_r($para);
		$sHtml = "<h3>正在跳转中 。。。</h3><form id='alipaysubmit' name='alipaysubmit' action='".$this->alipay_gateway_new."' method='".$method."'>";
